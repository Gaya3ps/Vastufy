...
function ProfileSidebar() {
  ...
  const sidebarItems = [
    ...
    { icon: FaComments, label: 'Chats', path: '/userChat/:chatId' },
  ...
  return (
    <div className="w-64 fixed top-0 h-full bg-gradient-to-b from-gray-100 to-gray-200 shadow-lg rounded-r-lg z-10">
      <div className="text-2xl font-semibold text-blue-800 p-6 border-b bg-white shadow-md">
        My Account
      </div>
      <ul className="space-y-2 mt-6 px-4">
        {sidebarItems.map((item, index) => {
          ...
          return (
            <li
              key={index}
              className={`flex items-center p-3 rounded-lg cursor-pointer transition-colors ease-in-out duration-300 ${
                isActive ? 'bg-blue-500 text-white' : 'bg-gray-100 hover:bg-gray-200'
              }`}
            >
              <Icon className={`${isActive ? 'text-white' : 'text-blue-800'} mr-3`} />
              <Link
                to={item.path}
